...
import { NjdotCrashesId, NjdotCrashesTable } from "@/src/njdot/table";
...
export const getServerSideProps: GetServerSideProps<Props, Params> = async ({ params, req, }) => {
    ...
    const spPerPage = parsePerPage(req, PerPageKey(NjspCrashesId), cookies)
    const dotPerPage = parsePerPage(req, PerPageKey(NjdotCrashesId), cookies)
    ...
    const [ spPage, njspProps, dotPage, yearStatsDicts, ] = await Promise.all([
        spDb.crashPage({ cc, mc, page, perPage: spPerPage, }),
        mn === null ? loadProps({ county: cn }) : Promise.resolve(null),
        dotDbs.crashPage({ cc, mc, page, perPage: dotPerPage, }),  // before: DOTEnd
